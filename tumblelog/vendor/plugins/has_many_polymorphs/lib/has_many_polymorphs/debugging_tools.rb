...
class << ActiveRecord::Base
  ...
  unless defined? GENERATED_CODE_DIR
    ...
    # you will get a folder "generated_models" in the plugin dir containing valid Ruby files
    ...
    GENERATED_CODE_DIR = "#{File.dirname(File.expand_path(__FILE__))}/../../generated_models"
    ...
    def method_missing(method_name, *args, &block)
      if COLLECTION_METHODS.include? method_name.to_sym
        ...
        # doesn't handle blocks cause we can't introspect on code like that in Ruby
...
# and if we want to track the reloader's shenanigans
#
