module ActiveRecord
  module Associations
    ...
    class PolymorphicAssociation < HasManyThroughAssociation
      ...
      def <<(*records)
        ...
        @reflection.klass.transaction do
          flatten_deeper(records).each do |record|
            ...
            next if @target.include? record and @reflection.options[:ignore_duplicates]
      ...
      def construct_scope
        ActiveRecord::Base.logger.warn "Warning; not all usage scenarios for polymorphic scopes are supported yet."
        super
      end
