...
module ActiveRecord::Associations
  module PolymorphicClassMethods
    ...
    RESERVED_KEYS = [:conditions, :order, :limit, :offset, :extend, :join_extend, :dependent, :rename_individual_collections]
    ...
    def acts_as_double_polymorphic_join options={}, &extension     
      ...
      collections.each do |association_id, children|
        ...
        parents.each do |parent_name| 
          ...
          parent_class = parent_name._as_class
    ...
    def create_has_many_polymorphs_reflection(association_id, options, &extension)
      options.assert_valid_keys(
        ...
        :ignore_duplicates, # default true, only affects the polymorphic collection
        :is_double,
...
#        :after_remove
      ...
      options[:ignore_duplicates] = true unless options.has_key? :ignore_duplicates
...
#        self._metaclass.send(:define_method, :reflection_options) do
#          reflection.options
#        end
        force_class_caching(association_id, reflection) if ActiveRecord::Associations::ClassMethods.has_many_polymorphs_cache_classes
        # set up the other related associations      
        create_join_association(association_id, reflection)
        create_virtual_associations_for_join_to_individual_children(association_id, reflection)
    ...
    def force_class_caching(association_id, reflection)
      # get our models out of the reloadable lists   
      begin
        all_classes_for(association_id, reflection).map(&:name).each do |klass_name|
          if (Dependencies.autoloaded_constants + Dependencies.explicitly_unloadable_constants).include? klass_name
            logger.debug "Ejecting #{klass_name.inspect} from the autoload lists"
            Dependencies.autoloaded_constants -= [klass_name]
            Dependencies.explicitly_unloadable_constants -= [klass_name]
          end
        end
      rescue NoMethodError
        raise PolymorphicError, "Rails 1.2 or later is required to set config.has_many_polymorphs_cache_classes = true" 
      end    
    end
...
#        :offset => reflection.options[:offset],
    ...
    def create_virtual_associations_for_join_to_individual_children(association_id, reflection)
      child_pluralization_map(association_id, reflection).each do |plural, singular|
        reflection.klass.instance_eval do

          association_name = singular.to_s
          association_name += "_as_#{association_id._singularize}" if reflection.options[:conflicts].include?(plural)
          association = demodulate(association_name)
          
          options = {:class_name => singular._classify, :foreign_key => reflection.options[:polymorphic_key]}
          (belongs_to association, options) unless self.reflect_on_all_associations.map(&:name).include? association          
        end
      end    
    end
    ...
    def create_has_many_through_associations_for_children_to_parent(association_id, reflection)
      ...
      child_pluralization_map(association_id, reflection).each do |plural, singular|
        raise PolymorphicError, "You can't have a self-referential polymorphic has_many :through without renaming the non-polymorphic foreign key in the join model." if singular == reflection.options[:as]
                
        parent = self
        singular._as_class.instance_eval do          
...
#              :offset => reflection.options[:offset],
    ...
    def create_has_many_through_associations_for_parent_to_children(association_id, reflection)
      child_pluralization_map(association_id, reflection).each do |plural, singular|
        ...
        type_selecter = "#{reflection.klass.table_name}.#{association_id._singularize}_type = '#{singular._as_class.base_class.name}'"
        has_many(current_association.to_sym, 
            ...
            :limit => reflection.options[:limit],
#        :offset => reflection.options[:offset],
            :order => devolve(association_id, reflection, reflection.options[:order], singular._as_class),
            :conditions => devolve(association_id, reflection, 
              (reflection.options[:conditions] ? "(#{reflection.options[:conditions]}) AND (#{type_selecter})" : type_selecter), 
              singular._as_class),
            :group => devolve(association_id, reflection, reflection.options[:group], singular._as_class)
            )
    ...
    def all_classes_for(association_id, reflection)
      klasses = [self, reflection.klass, *child_pluralization_map(association_id, reflection).values.map(&:_as_class)]
